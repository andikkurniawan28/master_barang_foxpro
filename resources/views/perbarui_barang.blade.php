...
@section('content')
    <div class="container-fluid">
        <h1 class="h3 mb-3"><strong>Perbarui Data Barang</strong></h1>
            ...
            <form id="formMasterBarang" action="{{ route('perbarui_barang.process', $id) }}" method="POST">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control form-control-sm" name="SAT"
                            value="{{ $barang->SAT }}" readonly>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" name="DISKRIPSI" readonly>{{ $barang->DISKRIPSI }}</textarea>
                </div>
                    ...
                    <div class="col-md-4">
                        <select class="form-select form-select-sm select2" name="d6" id="d6">
                            ...
                            {{-- @foreach ($d6_data as $item)
                                <option value="{{ $item->D6 }}">
                                    {{ $item->D6 }} | {{ $item->KET }} | {{ $item->D5 }}
                                </option>
                            @endforeach --}}
                        </select>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            setInterval(function() {
                ...
                console.log('🔄 Memuat data D6, D8, D10 untuk:', d1sampaid5);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        let $d6 = $('#d6');
                        $d6.empty();
                        if (response.d6 && response.d6.length > 0) {
                            $.each(response.d6, function(i, item) {
                                $d6.append(
                                    $('<option>', {
                                        value: item.D6,
                                        text: item.D6 + ' | ' + item.KET
                                    })
                                );
                            });
                        } else {
                            $d6.append('<option value="">-- Tidak ada data D6 --</option>');
                        }
                        $d6.trigger('change.select2');

                        // --- D8 ---
                        let $d8 = $('#d8');
                        $d8.empty();
                        if (response.d8 && response.d8.length > 0) {
                            $.each(response.d8, function(i, item) {
                                $d8.append(
                                    $('<option>', {
                                        value: item.D8,
                                        text: item.D8 + ' | ' + item.KET
                                    })
                                );
                            });
                        } else {
                            $d8.append('<option value="">-- Tidak ada data D8 --</option>');
                        }
                        $d8.trigger('change.select2');

                        // --- D10 ---
                        let $d10 = $('#d10');
                        $d10.empty();
                        if (response.d10 && response.d10.length > 0) {
                            $.each(response.d10, function(i, item) {
                                $d10.append(
                                    $('<option>', {
                                        value: item.D10,
                                        text: item.D10 + ' | ' + item.KET
                                    })
                                );
                            });
                        } else {
                            $d10.append('<option value="">-- Tidak ada data D10 --</option>');
                        }
                        $d10.trigger('change.select2');

                        console.log('✅ D6, D8, D10 berhasil diperbarui.');
                    },
                    error: function(xhr) {
                        console.error('❌ Gagal ambil data D6/D8/D10:', xhr.responseText);
                    },
                });
